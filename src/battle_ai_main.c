...
// the following checks apply to any target (including user)
    ...
    {
        if (gBattleWeather & B_WEATHER_PRIMAL)
        {
            switch (move)
            {
                case MOVE_SUNNY_DAY:
    ...
    // other
        ...
        case EFFECT_SANDSTORM:
            if (gBattleWeather & (B_WEATHER_SANDSTORM | B_WEATHER_PRIMAL)
             || PartnerMoveEffectIsWeather(AI_DATA->battlerAtkPartner, AI_DATA->partnerMove))
                score -= 8;
            break;
        case EFFECT_SUNNY_DAY:
            if (gBattleWeather & (B_WEATHER_SUN | B_WEATHER_PRIMAL)
             || PartnerMoveEffectIsWeather(AI_DATA->battlerAtkPartner, AI_DATA->partnerMove))
                score -= 8;
            break;
        case EFFECT_RAIN_DANCE:
            if (gBattleWeather & (B_WEATHER_RAIN | B_WEATHER_PRIMAL)
             || PartnerMoveEffectIsWeather(AI_DATA->battlerAtkPartner, AI_DATA->partnerMove))
                score -= 8;
            break;
        case EFFECT_HAIL:
            if (gBattleWeather & (B_WEATHER_HAIL | B_WEATHER_PRIMAL)
             || PartnerMoveEffectIsWeather(AI_DATA->battlerAtkPartner, AI_DATA->partnerMove))
                score -= 8;
            break;
        case EFFECT_ATTRACT:
